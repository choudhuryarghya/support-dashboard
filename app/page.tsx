  ...
  const fetchAllMessages = useCallback(async () => {
    ...
    const previews: ConversationPreview[] = Array.from(convMap.entries())
      .map(([uid, info]) => {
        ...
        // Find matching ticket by checking all emails in the thread
        let matchedTicket: Ticket | undefined
        let bestEmail: string | null = null

        // 1. Check for specific emails extracted from messages
        for (const email of Array.from(info.emails)) {
          const t = Array.from(tickets.values()).find(t => t.user_email === email)
          if (t) {
            matchedTicket = t
            bestEmail = email
            break
        ...
        if (!matchedTicket) {
          ...
          const t = Array.from(tickets.values()).find(t => t.user_email === internalEmail)
          if (t) {
            matchedTicket = t
            bestEmail = internalEmail
          }
        }

        // If no match found, just take the first email if any exist
        if (!bestEmail && info.emails.size > 0) {
          bestEmail = Array.from(info.emails)[0]
        }
        ...
        return {
          ...
          status: matchedTicket?.status ?? "open", // Default to open if no ticket
          ticket_id: matchedTicket?.ticket_id ?? null
        }
      })
